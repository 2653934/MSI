#!/bin/bash
#SBATCH --job-name=spatial-cac-val
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/spatial-cac-val-%j.out
#SBATCH --error=logs/spatial-cac-val-%j.err
...
case "$DATASET" in
    ...
    *)
        echo "Unknown CAC validation dataset: $DATASET" >&2
        exit 2
        ;;
esac
case "$VARIANT" in
...
CHECKPOINT_OUTPUT="/datasets/zsuliman/msi_checkpoints/spatial_msipl/cac/$CHECKPOINT_GROUP/${DATASET}_seed1/$VARIANT"
...
python -c 'import sys, torch; sys.exit(0 if torch.cuda.is_available() else "CUDA is unavailable; CAC validation training stopped before loading the model.")'
...
if [ -f "$LATEST_CHECKPOINT" ]; then
    ...
    echo "Starting frozen CAC validation run: $DATASET $VARIANT"
fi
