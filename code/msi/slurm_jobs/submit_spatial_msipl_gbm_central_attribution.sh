...
counts=(530 458 453 478 589 464 686 523)
...
for index in "${!datasets[@]}"; do
    ...
    count="${counts[$index]}"
    summary="$PROJECT_ROOT/results/experiments/spatial_msipl_attributed_peak_evaluation/${dataset}_seed1/central_only/summary.json"
    ...
    gate_submission=$(sbatch \
        ...
        --time=00:01:00 \
        --mem=256M \
        --output="logs/igc-gate-%j.out" \
        --error="logs/igc-gate-%j.err" \
        --wrap='true')
    ...
    if ! worker_submission=$(sbatch \
        ...
        --output="logs/gbm-ig-central_only-%j.out" \
        --error="logs/gbm-ig-central_only-%j.err" \
...
if (( ${#gate_ids[@]} > 0 )); then
    ...
    summary_submission=$(sbatch --dependency="afterok:$dependency" \
        slurm_jobs/summarise_spatial_msipl_gbm_context_attribution.sh)
else
...
echo
echo "Submitted ${#worker_ids[@]} incomplete centre-only attribution controls."
