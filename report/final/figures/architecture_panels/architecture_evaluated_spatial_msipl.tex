...
\begin{tikzpicture}[font=\sffamily]
  ...
  % These three alternatives are deliberately separated vertically.  The
  % shared block style is taller than the old centre spacing, which caused
  % their outlines and labels to overlap in the rendered panel.
  \node[block, fill=black!3, text width=1.62cm, minimum height=1.05cm]
    (central) at (3.10,0.86)
    {centre-only\\control};
  \node[block, fill=blue!6, text width=1.62cm, minimum height=1.05cm]
    (uniform) at (3.10,-0.42)
    {uniform\\neighbour mean};
  \node[block, fill=purple!6, text width=1.62cm, minimum height=1.05cm]
    (attention) at (3.10,-1.70)
    {learned\\attention};
